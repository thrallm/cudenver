...
void selectionSortArray(int [], int, char);
...
int main(){
	...
	selectionSortArray(values,SIZE, DIRECTION);
...
void selectionSortArray(int array[], int elems, char direction){
						...
						// and then increments seek
	{
		...
		array[seek] = minValue;
		displayArray(array, elems); // display the array at each step
	}
}
// Exercise 1: Program is modified to allow sorting ascending or descending.
//
// Exercise 2:
// The program is modified to output after each step. The output matches what I did by hand.
